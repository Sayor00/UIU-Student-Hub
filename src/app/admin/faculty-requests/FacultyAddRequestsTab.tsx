...
export default function FacultyAddRequestsTab() {
    ...
    const fetchRequests = React.useCallback(async (pageNum: number) => {
        setLoading(true);
        try {
            const res = await fetch(`/api/admin/faculty-requests?status=${statusFilter}&limit=20&page=${pageNum}`);
            const data = await res.json();
            if (res.ok) {
                if (pageNum === 1) {
                    ...
                    setRequests((prev) => {
                        ...
                        const uniqueNew = (data.requests || []).filter((item: any) => !existingIds.has(item._id));
